...
async def read_data_in_with_dest_clk(dut):
    ...
    if int(dut.g_SRC_INPUT_REG.value) == 1:
    ...
    for i in range(TOTAL_DATA_IN*CLK_RATIO_CEIL):
        ...
        if int(dut.g_SRC_INPUT_REG.value) == 1:
            ...
            input_data.append(int(dut.src_in.value))
...
async def read_data_out(dut):
    ...
    while data_read == "X":
        await RisingEdge(dut.dest_clk)
        data_read = str(dut.dest_out.value)
    output_data.append(int(dut.dest_out.value))        
